...
// 질문은 **우리 설계 도구**다 — 순서·묶음은 data-q(DOM)로 강제하되 문구는 화면에 내지 않는다.
// 창업자 지시 2026-07-29: Q번호도, 질문 제목도 전부 삭제. 섹션 제목이 이미 무슨 얘긴지 말한다.
t('① 화면에 Q번호 배지 없음', !/class="q-no"/.test(html) && !/>Q[0-4]</.test(html));
// 주석에는 남아 있어야 한다 — 질문 순서는 우리 설계 근거다("당신과 나만 알면 되는" 부분).
// 화면에 나가는 문자열(태그 안 텍스트)에서만 금지한다.
const visible = html.replace(/<!--[\s\S]*?-->/g, '').replace(/^\s*\/\/.*$/gm, '');
t('① 질문 제목 문구가 화면에 없음', !/q-head/.test(html)
  && !['이번 회차에 팔릴까', '얼마에 살 수 있나', '그게 싼 건가', '왜 아직 안 팔렸나', '그럼 뭘 확인해야 하나']
       .some(q => visible.includes(q)));
t('① "판단 준비도" 제목도 삭제(배지가 이미 신호를 말한다)', !/<b>판단 준비도<\/b>/.test(html));
...
t('① 전문가가 Q4에', qIdx(4) < at('sec-agents'));
// 유사물건은 '이 물건'이 아니라 '다른 물건' 얘기라 어떤 질문의 답도 아니다 → 묶음 밖 꼬리
t('① 유사물건은 질문 묶음 밖', at('sec-sim') > html.indexOf('<!-- /Q4 -->'));
// 같은 얘기를 여러 섹션이 되풀이하지 않는가 — 경쟁평가에서 권리·유찰 항목 제거
t('① 경쟁평가가 권리 리스크를 되풀이하지 않음', !/lbl:'권리 리스크'/.test(html));
t('① 경쟁평가가 유찰 이력을 되풀이하지 않음', !/lbl:'유찰 이력 분석'/.test(html));
...
// ── ⑦ 실화면 점검에서 나온 교정 (창업자 스크린샷 2026-07-29) ──
// 화면 문자열에서만 금지한다(왜 뺐는지 설명하는 주석은 남아 있어야 한다)
t('⑦ 공매에 없는 "매각기일" 표기 제거',
  !/매각기일 —<|매각기일 \$\{|type:'매각기일/.test(html) && !/>\s*매각기일/.test(html));
t('⑦ 법원경매 어휘(지방법원·타경) 잔존 0', !/지방법원|타경/.test(html));
t('⑦ 데모도 공매 어휘 — 사건번호가 아니라 물건관리번호', /const caseNoLabel='물건관리번호'/.test(html));
t('⑦ 임대차·권리에 감정평가액 카드 없음(가격은 그리드 소관)', !/rt">감정평가액</.test(html));
t('⑦ 사용자가 할 수 없는 일은 "다음 확인"에 넣지 않음',
  !/next\.push\(\{label:'예측은 개찰 전에만 봉인합니다/.test(html));
t('⑦ 지역 통계는 "지역 전체"로 범위 명시(이 물건 예보와 구분)',
  /지역 전체 낙찰률/.test(html) && /지역 평균 경쟁률/.test(html));
t('⑦ 지역 통계 각주가 이 물건 예보와 다름을 밝힘', /이 물건 자체의 예보[\s\S]{0,40}따로 있습니다/.test(html));
