...
- `netlify/functions/onbid-detail.js` — Serverless proxy for the "온비드 부동산 물건상세 조회 서비스" (Base URL `https://apis.data.go.kr/B010003/OnbidRlstDtlSrvc2`, operation `GET /getRlstDtlInf2`, looked up by `cltrMngNo`+`pbctCdtnNo`). Service applied and approved on data.go.kr; the proxy is wired but `mapDetail()` still returns raw response — **remaining step**: set `ONBID_DETAIL_API_URL` env var in Netlify, then inspect the first live response via `?debug=1` and fill in field mappings. Returns a clean 501 when the env var is unset. `bidcast-detail.html` calls this function asynchronously for live items and gracefully falls back to the sessionStorage snapshot on any failure.
...
**Detail API wired (pending env var):** the "온비드 부동산 물건상세 조회 서비스" (`/getRlstDtlInf2`) is now proxied via `onbid-detail.js` and called asynchronously from `bidcast-detail.html` for live items. `mapDetail()` currently returns raw response — after `ONBID_DETAIL_API_URL` is set in Netlify and the first live response is inspected via `?debug=1`, the field mappings in both the proxy and the front-end enrichment code should be refined. `bidcast-list.html`'s AI report modal still uses generated/example detail data, not live per-item detail.
