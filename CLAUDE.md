...
- `netlify/functions/onbid-bidinfo.js` — Proxy for the "차세대 온비드 물건상세 입찰정보 조회서비스" (data.go.kr dataset 15157251): per-item 회차별 입찰정보 — the raw source for 낙찰 결과/낙찰가 data that prediction statistics will be built on. **Pending env var** `ONBID_BIDINFO_API_URL` (clean 501 until set; op default `/getDtlBidInf2` inferred from the 차세대 naming pattern, override via `ONBID_BIDINFO_API_OP`; verify with `?debug=1` — the `winAmt` field-name candidates in `mapRound()` are unconfirmed guesses). `bidcast-detail.html` calls it for live items and swaps the 입찰 이력 timeline to real rounds on success (snapshot content kept on any failure).
