...
**Confirmed 2026-07-16:** the user's approved service is `한국자산관리공사_차세대 온비드 부동산 물건목록 조회서비스`, End Point `https://apis.data.go.kr/B010003/OnbidRlstListSrvc2` (JSON+XML) — this is the "차세대"/#15157207 generation, not the older #15000851 one. Set `ONBID_API_URL` to this End Point.
...
- The operation path appended to the End Point (e.g. `/getOnbidRlstList`) and the exact request parameter names — see the service's "활용신청 상세기능정보" section / the linked `OpenAPI활용가이드` doc on data.go.kr.
