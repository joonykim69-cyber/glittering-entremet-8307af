...
**Confirmed 2026-07-16:** the user's approved service is `한국자산관리공사_차세대 온비드 부동산 물건목록 조회서비스` v1.0.0, Base URL `https://apis.data.go.kr/B010003/OnbidRlstListSrvc2` (JSON+XML) — this is the "차세대"/#15157207 generation, not the older #15000851 one. Its single operation, `GET /getRlstCltrList2`, is now hardcoded as `ONBID_OPERATION` in `onbid-search.js` and appended to the `ONBID_API_URL` env var (which should hold only the Base URL) at request time. Two response identifier fields are confirmed from the Swagger description: `cltrMngNo` (물건관리번호) and `pbctCdtnNo` (공매조건번호) — these are wired into `mapOnbidItem()`. The response schema uses camelCase (unlike the older API generation's UPPER_SNAKE_CASE), so the rest of `mapOnbidItem()`'s field guesses were updated to camelCase equivalents (`cltrNm`, `ldnmAdrs`, `aprslAsesAvgAmt`, etc.) — still unverified, but a better-informed guess than before.

**What's still NOT confirmed** (data.go.kr blocks automated fetches with a 403, so this couldn't be verified against the live Swagger doc while building it):
- The Swagger doc lists two **required** search params — `prptDivCd` (재산유형코드) and `pvctTrgtYn` (수의계약가능여부) — but their valid code values are unknown. Calling the API without them may fail with a missing-required-param error. Check the operation's "Parameters" table in the Swagger UI (or the `OpenAPI활용가이드` doc) for the code table, then pass them through `onbid-search.js`'s `qs.prptDivCd`/`qs.pvctTrgtYn` passthrough (already wired, just needs valid values from the front end or a sensible default per asset-type bucket).
- The rest of the response field names inside `mapOnbidItem()` are still best-effort camelCase guesses, not verified against a live response — expand the `getRlstCltrList2_response` model in the Swagger "Models" section (or the guide doc) to get the authoritative list.
- A separate "온비드 부동산 물건상세 조회 서비스" apparently exists (referenced in this service's description, looked up via `cltrMngNo`/`pbctCdtnNo`) for per-item detail — not yet integrated; `bidcast-list.html`'s report modal still uses generated/example detail data.
